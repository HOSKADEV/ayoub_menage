...
class DatatablesController extends Controller
{
  ...
  public function products(Request $request){
    ...
    return datatables()
      ...
      ->addColumn('price', function ($row) {
        ...
        return number_format($row->unit_price,2,'.',',') . ' Mru';
  ...
  public function orders(Request $request){
    ...
    return datatables()
          ...
          if(!in_array($row->status,['pending','canceled'])){
            if(!is_null($row->invoice)){
              ...
              href="https://api.whatsapp.com/send?text='.
                ...
                __('Total amount').': '.number_format($row->invoice->total_amount,2,'.',','). ' Mru %0A'.
      ...
      ->addColumn('purchase_amount', function ($row) {
        ...
        if(!is_null($row->invoice)){
          return number_format($row->invoice->purchase_amount,2,'.',','). ' Mru';
        }
      ...
      ->addColumn('tax_amount', function ($row) {
        ...
        if(!is_null($row->invoice)){
          return number_format($row->invoice->tax_amount,2,'.',','). ' Mru';
        }
      ...
      ->addColumn('total_amount', function ($row) {
        ...
        if(!is_null($row->invoice)){
          return number_format($row->invoice->total_amount,2,'.',','). ' Mru';
        }
  ...
  public function wilayas(Request $request)
  {
    ...
    return datatables()
          ...
          {
            return number_format($row->delivery_price,2,'.',','). ' Mru';
          })
