...
class OrderController extends Controller
{
  ...
  public function create(Request $request)
  {
    ...
    try
    {
      ...
      // $order = Order::create($request->except('image'));
      $order = Order::create([
      ...
      if ($request->hasFile('image'))
      {
        ...
        $filename = 'orders/' . $order->id . '/' . md5(time().$name) . '.' . $extension;
    ...
    try {
      ...
      if($request->has('status')){
        ...
        $this->send_fcm_device(
          ...
          __('Your order has been '.$request->status),
